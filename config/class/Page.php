...
写一个方法 返回一个生成的样式页码（并且可以根据自己需要添加样式）
...
class Page{
    ...
    /**
     * pageStyle1 分页样式（通用样式）
     * 样式 总共搜索到45条记录,每页显示10条,当前第1/4页 [首页] [上页] [1] [2] [3] .. [下页] [尾页] 
     * @return string 
     */
    protected function pageStyle1(){
        /* 构造普通模式的分页 
        总共搜索到4523条记录,每页显示10条,当前第1/453页 [首页] [上页] [1] [2] [3] .. [下页] [尾页] 
        */
        $pageStr='找到'.$this->count.'条记录，每页'.$this->subPages.'条，';
        $pageStr.='第'.$this->currPage.'页/共'.$this->countPages.'页。';
	...
	/**
     * pageStyle2 简式下拉提示Ajax分页(会计科目下拉提示分页)
     * 样式 共10条记录，第1/2页，[首页][下页] [尾页]
     * @return string 
     */
    protected function pageStyle2(){
		
		$pageStr='共'.$this->count.'条记录，';
        $pageStr.='第'.$this->currPage.'/'.$this->countPages.'页，';
 
        $_GET['page'] = 1;
		
        $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showAccountPages(this);return false;">首页</a>]</span>';
        //如果当前页不是第一页就显示上页
        if($this->currPage>1){
            $_GET['page'] = $this->currPage-1;
            $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showAccountPages(this);return false;">上页</a>]</span>';
        }
 
        //如果当前页小于总页数就显示下一页
        if($this->currPage<$this->countPages){
            $_GET['page'] = $this->currPage+1;
            $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showAccountPages(this);return false;">下页</a>]</span>';
        }
 
        $_GET['page'] = $this->countPages;
        $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showAccountPages(this);return false;">尾页</a>]</span>';
	...
	/**
     * pageStyle3 简式下拉提示Ajax分页(核算项目下拉提示分页)
     * 样式 共10条记录，第1/2页，[首页][下页] [尾页]
     * @return string 
     */
    protected function pageStyle3(){
        ...
        $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showXMPages(this);return false;">首页</a>]</span>';
        //如果当前页不是第一页就显示上页
        if($this->currPage>1){
            $_GET['page'] = $this->currPage-1;
            $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showXMPages(this);return false;">上页</a>]</span>';
        }
 
        //如果当前页小于总页数就显示下一页
        if($this->currPage<$this->countPages){
            $_GET['page'] = $this->currPage+1;
            $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showXMPages(this);return false;">下页</a>]</span>';
        }
 
        $_GET['page'] = $this->countPages;
        $pageStr.='<span>[<a href="'.$this->href.'?'.http_build_query($_GET).'" onclick="showXMPages(this);return false;">尾页</a>]</span>';
