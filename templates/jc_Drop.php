...
if($info){ //如果存在数据；
?>
<form id="jc_DropForm" action="templates/jc_DropSubmit.php" method="get">
...
$(function(){
	...
	$("#jc_DropForm").ajaxForm(options); 

	//分类下拉提示；
	$("#jc_DropForm input[name=typec]").getSuggestBox({RequestURI:"templates/jc_typec.php",FindSource:$("#jc_DropForm input[name=tbl]").val(),Callback:function(arrtd){
		$("#jc_DropForm input[name=typec]").val(arrtd[0]);
	}});
});
// pre-submit callback 
