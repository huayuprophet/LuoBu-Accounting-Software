...
if($info){ //如果存在数据；
?>
<form id="jc_CopyForm" action="templates/jc_InsertSubmit.php" method="get">
...
<tr>
<td>ID</td><td><input name="id" type="text" maxlength="12" style="width:550px" value="<?php echo $info["id"]; ?>" required />
...
<input name="insert" type="submit" value="复制" />
</td>
</tr>
</table>
</form>
...
$(function(){
	...
	$("#jc_CopyForm").ajaxForm(options); 

	//分类下拉提示；
	$("#jc_CopyForm input[name=typec]").getSuggestBox({RequestURI:"templates/jc_typec.php",FindSource:$("#jc_CopyForm input[name=tbl]").val(),Callback:function(arrtd){
		$("#jc_CopyForm input[name=typec]").val(arrtd[0]);
	}});
});
// pre-submit callback 
