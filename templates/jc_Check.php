...
if($info){ //如果存在数据；
?>
<form id="jc_CheckForm" action="templates/jc_CheckSubmit.php" method="get">
...
<tr>
<td>ID</td><td><input name="id" type="text" maxlength="12" style="width:550px" readonly value="<?php echo $info["id"]; ?>" required />
...
<input name="check1" type="submit" value="审核" />
<input name="check2" type="submit" value="反审核" />
<input name="check3" type="submit" value="全部审核" />
<input name="check4" type="submit" value="全部反审核" />
</td>
</tr>
</table>
</form>
...
$(function(){
	...
	$("#jc_CheckForm").ajaxForm(options); 

	//分类下拉提示；
	$("#jc_CheckForm input[name=typec]").getSuggestBox({RequestURI:"templates/jc_typec.php",FindSource:$("#jc_CheckForm input[name=tbl]").val(),Callback:function(arrtd){
		$("#jc_CheckForm input[name=typec]").val(arrtd[0]);
	}});
});
// pre-submit callback 
