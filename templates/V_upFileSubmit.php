...
if(empty($_FILES["up_file"]["name"])){
	...
	for($i=0;$i<count($fileinfo["name"]);$i++){
		...
		$pzh = mysqli_real_escape_string($mysqli,$_POST["pzh"]);
		//获取文件的扩展名；
		...
		if(!in_array($kzm,$arrayFileType,true)){
			...
			//上传文件新的名称由期间，凭证号，时间，操作员，序号四个部分组成，每个部分使用下划线区分；
			//时间与序号是区分不同文件的关键变量，时间可能会重复，但是加上了序号，重复的机率变得更小。
			$newName=$qj.'_'.$pzh.'_'.@strtotime("now").'_'.md5($_SESSION["username"]).'_'.$i.'.'.$kzm;
			...
			if($fileinfo['size'][$i]<104857600 && $fileinfo['size'][$i]>0){
				...
				if($upFileOk){
					...
					$strsql='insert into v1_fj(fileaddress,filename,qj,pzh) values("'.$newName.'","'.$fileinfo["name"][$i].'","'.$qj.'","'.$pzh.'")';
					$mysqli->query($strsql);
