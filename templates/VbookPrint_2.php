...
if(($_SERVER['REQUEST_METHOD']=='GET')&&isset($_GET["qj"])&&isset($_GET["pzh"])&&isset($_GET["xh"])){//生成查询页面；
	...
	echo '<h2>欢迎使用出库单据打印功能(遵循先借方后贷方正向规则)<br/><input id="print_VbookPrint_1" type="button" value="打印预览" /></h2>';
	...
	$strsql='select ywrq,lrrq,qj,pzh,xh,zy,km,kmF,dr,cr,slwb,slwbF,kh,khF,ch,chF,etr,chk,acc,fj from V1 where qj="'.$qj.'" and pzh="'.$pzh.'" order by xh asc';
	
	$sql=$mysqli->query($strsql);
    $info=$sql->fetch_array(MYSQLI_BOTH);
	
	$strkh='客户：'.$info["khF"];//供应商信息变量；
	$strywrq='发货日期：'.$info["ywrq"];//业务日期变量；
	$strdh='单号：'.$info["qj"].'-'.$info["pzh"];//单号信息变量，期间+凭证号；
	
	echo '<div id="printDJ_VbookPrint_1">';
	
	$i=1;//序号；
	$sumSlwb=0;$sumCr=0;//数量及金额合计；
	...
	echo '<tr><td colspan="6" align="center"><font size="6">出库单据</font></td></tr>';
	
	//表头相关信息；
	echo '<tr><td colspan="3" align="left">'.$strkh.'</td><td colspan="2" align="left">'.$strywrq.'</td><td align="center">'.$strdh.'</td></tr>';
	...
	do{
		if(is_null($info["km"])){
			...
			echo '<td align="center">'.round($info["cr"],2).'</td>';
			echo '<td>'.$info["zy"].'</td>';
			echo '</tr>';
			$sumSlwb+=$info["slwb"];
			$sumCr+=$info["cr"];
		...
		if($i%5==0){
			...
			echo '<td align="center"></td>';
			echo '<td align="center">'.round($sumCr,2).'</td>';
			echo '<td align="center"></td>';
			echo '</tr>';
	...
	echo '<h2>欢迎使用出库单据打印功能页面<br/>未传入合法参数！</h2>';
}
echo '</div>';
?>
