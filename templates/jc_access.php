...
if($_SERVER['REQUEST_METHOD']=='GET'){
	...
	switch($tj){
		case "包含":
			$strsql='select * from '.$tblName.' where '.$zd.' like "%'.$gjz.'%" order by id asc';
			break;
		case "开头是":
		    $strsql='select * from '.$tblName.' where '.$zd.' like "'.$gjz.'%" order by id asc';
			break;
		case "结尾是":
			...
			break;
		case "等于":
			$strsql='select * from '.$tblName.' where '.$zd.'="'.$gjz.'" order by id asc';
			break;
		default://非以上结果的处理；
	...
	echo '<div align="left">';
	echo '<table id="tbl1_jc_access" class="mytable">';
	...
	if($info){ //如果存在数据；
		...
		DO{
		   ...
		   echo '<td>'.$info["typec"].'</td>';
		   echo '<td align="center">'.$info["fj"].'</td>';
		   $strURL='tbl='.$tblName.'&id='.$info["id"];//只提供了参数部分；
		...
		echo '<tr>';
		echo '<td align="center">系统未检索到相关数据；</td>';
		echo '</tr>';
	}
...
$(function(){
	//表格插件应用
	$("#rightHand>div:visible #tbl1_jc_access").mytable({a:true,b:true,c:true,d:false});
	//分页跳转
	...
	//基础信息停用操作
	$("#rightHand>div:visible #tbl1_jc_access a").on("click",function(){
		strURL='templates/jc_access_ty.php?'+$(this).attr("href");
